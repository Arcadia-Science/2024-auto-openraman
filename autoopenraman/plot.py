...
class SpectrumPlotter:
    def __init__(self, input_path: Path):
        ...
        Uses Plotly to display spectra of a single CSV file or directory. CSV files should have a
        header row and two columns. The first column is assumed to be the X-axis (could be
        wavelength, wavenumber, pixels) and the second column is the Y-axis (Intensity). These files
        are generated by `autoopenraman.acq.AcquisitionManager`.
    ...
    def _get_csv_files(self) -> list[Path]:
        ...
        if self.input_path.is_dir():
            return list(self.input_path.glob("*.csv"))
